...
parser.add_argument('--use_wham_noise', action='store_true',
                    default=False,
                    help='Set True to mix WHAM! noise (default False)')
parser.add_argument('--wham_dir', type=str, required=False,
...
parser.add_argument('--freqs', nargs='+', default=['16k'],
                    help='(fixed) only 16k mixes are generated')
parser.add_argument('--modes', nargs='+', default=['min'],
                    help='(fixed) only min-mode is generated')
parser.add_argument('--types', nargs='+', default=['mix_clean'],
                    help='(fixed) only clean mixtures')
...
def read_sources(row, n_src, librispeech_dir, wham_dir):
    ...
    # New – skip noise when wham_dir is empty OR flag is False
    if wham_dir == "" or not use_wham_noise:
        noise = np.zeros(max_length, dtype='float32')          # silent filler
    else:
